...
describe('NFT Discount Calculator', () => {
  ...
  describe('calculateBulkPrice', () => {
    ...
    it('should calculate total with cafe discount', () => {
      const items = [{ price: 10 }, { price: 15 }, { price: 25 }]
  ...
  describe('Integration Scenarios', () => {
    ...
    it('should correctly calculate cafe order with multiple items', () => {
      const items = [
        { price: 5.5, quantity: 2 }, // Coffee x2
        { price: 12.0, quantity: 1 }, // Sandwich
        { price: 3.5, quantity: 3 }, // Pastry x3
      ]

      const result = calculateBulkPrice(items, 'cafe', true)

      const expectedTotal = 5.5 * 2 + 12.0 + 3.5 * 3 // 33.50
      expect(result.originalPrice).toBeCloseTo(33.5, 2)
