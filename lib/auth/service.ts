...
export async function registerUser(input: RegisterInput): Promise<AuthResult> {
  try {
    ...
    if (!passwordValidation.isValid) {
      throw new AuthenticationError('INVALID_PASSWORD', passwordValidation.errors.join(', '), 400)
    }

    // Check if user already exists
    const checkQuery = 'SELECT id FROM users WHERE email = $1'
    const { data: existingUser } = await executeQuerySingle<{ id: string }>(checkQuery, [
    ...
    const { data: userData, error: dbError } = await executeQuerySingle<DbUser>(insertQuery, [
      input.email.toLowerCase(),
      passwordHash,
      input.fullName || null,
      input.phone || null,
    ...
    if (dbError || !userData) {
      ...
      throw new AuthenticationError('REGISTRATION_FAILED', 'Failed to create user profile', 500)
    }
...
export async function loginUser(input: LoginInput): Promise<AuthResult> {
  try {
    ...
    const { data: userData, error: dbError } = await executeQuerySingle<DbUser>(query, [
      input.email.toLowerCase(),
    ])
...
export async function initiatePasswordReset(
  ...
  try {
    ...
    const checkQuery = 'SELECT id FROM users WHERE email = $1'
    const { data: user } = await executeQuerySingle<{ id: string }>(checkQuery, [
      email.toLowerCase(),
    ])
    ...
    const { error: insertError } = await executeQuery(insertQuery, [
      user.id,
      resetToken,
...
export async function resetPassword(
  ...
  try {
    ...
    const { error: updateError } = await executeQuery(updateQuery, [
      passwordHash,
      tokenData.user_id,
    ])
